...
<script>
	import upload_mixin from '@/mixin/upload'
	export default {
		mixins: [upload_mixin],
		data() {
			return {
		...
		methods: {
				...
				uni.chooseImage({
					...
					success: async (res) => {
						...
						for (let i = 0, len = res.tempFiles.length; i < len; i++) {
							...
							// 	uni.showModal({
							// 		title: '提示',
							// 		content: `第${[...new Set(this.upload_exceeded_list)].join(',')}张图片超出限制${this.upload_max}M,已过滤`,
							// 		showCancel: false,
							// 		confirmText: '确认',
							// 		success(res) {
							// 			if (res.confirm) {
			...
			async upload(upload_auto) {
				upload_auto ? await this.upImage(this.upload_cache) : console.warn(
	...
	const upload = options => {
		...
		uploadTask.onProgressUpdate(async (res) => {
			...
			this.upload_before_list = this.upload_before_list;
		});
	}
</script>
